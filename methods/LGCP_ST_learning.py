...
def LGCP_ST_learning(xtrain, xtest, ytrain, task_features, kernel_type, spatial_kernel, temporal_kernel, point_estimate, 
                     ytrain_non_missing_index, sparsity, sparsity_level, inducing_on_inputs, optim_ind, offset_type, 
                     trainable_offset, lengthscale_initial, sigma_initial, white_noise, input_scaling, lengthscale_initial_weights, 
                     sigma_initial_weights, prior_mixing_weights, num_samples_ell, epochs, var_steps, display_step_nelbo, 
                     intra_op_parallelism_threads, inter_op_parallelism_threads, task, fold = 0, num_features = 2, events_location = None):
    
	t = task

	kernel_inducing = xtrain# Get the dimension of the inputs
	...
	if dim_inputs == 3:
		xtrain = np.concatenate((xtrain[:,0][:,np.newaxis][ytrain_non_missing_index[:,t]], xtrain[:,1][:,np.newaxis][ytrain_non_missing_index[:,t]], xtrain[:,2][:,np.newaxis][ytrain_non_missing_index[:,t]]), axis = 1)#xtrain = xtrain[ytrain_non_missing_index[:,t]]
	if dim_inputs == 2:
	...
	optimizer = tf.compat.v1.train.AdamOptimizer(0.0025)#optimizer = tf.train.RMSPropOptimizer(0.005)
   #optimizer = tf.compat.v1.train.AdamOptimizer(0.0025)
