...
def eye(N):
    return tf.linalg.tensor_diag(tf.ones(tf.stack([N, ]), dtype='float32'))
...
def log_cholesky_det(chol):
    return 2 * tf.reduce_sum(tf.math.log(tf.linalg.tensor_diag_part(chol)))
...
def logsumexp(vals, dim=None):
    ...
    if dim is None:
        return m + tf.math.log(tf.reduce_sum(tf.exp(vals - m), dim))
    else:
        return m + tf.math.log(tf.reduce_sum(tf.exp(vals - tf.expand_dims(m, dim)), dim))
