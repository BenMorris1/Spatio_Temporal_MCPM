...
class Periodic(kernel.Kernel):
    ...
    def __init__(self, period=1.0, variance=1.0, lengthscale=1.0, white =0.01):
        ...
        self.variance = tf.Variable([variance], dtype=tf.float32, name = 'variance', trainable=False)

        self.lengthscale = tf.Variable([lengthscale], dtype=tf.float32, name = 'lenghtscale', trainable=False)

        self.period = tf.Variable([period], dtype=tf.float32, name = 'period', trainable=False)
        self.white = white
    ...
    def kernel(self, points1, points2=None):
        ...
        r = np.float64(np.pi) * (f - f2) / self.period
