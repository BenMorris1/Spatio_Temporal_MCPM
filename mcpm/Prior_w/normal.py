...
class Normal(prior_w.Prior_w):
    ...
    def build_samples(self, num_samples, means, covar):
        ...
        var = tf.transpose(tf.linalg.diag_part(covar))
        means = tf.transpose(means)
        return (means + tf.sqrt(var) * tf.random.normal([num_samples, self.num_tasks, self.num_latent], seed=5))
    
    def entropy(self, means, covar):
        sum_val = 0.0
        # This function is building the entropy for the weights - eq.
        for i in range(self.num_latent):
            chol = tf.linalg.cholesky(covar[i])
            trace = tf.reduce_sum(tf.linalg.diag_part(tf.linalg.cholesky_solve(chol,covar[i])))
    ...
    def cross_entropy(self, means, covar, chol_var_weights, kernel_chol_weights):
        sum_val = 0.0
        for i in range(self.num_latent):
            trace = tf.reduce_sum(tf.linalg.diag_part(tf.linalg.cholesky_solve(chol_var_weights[i],covar[i])))
