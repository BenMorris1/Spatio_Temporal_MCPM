...
from . import prior_w
from mcpm.util import normals
from mcpm.util import util
...
class GP(prior_w.Prior_w):
    ...
    def entropy(self, means, covar):
        ...
        for i in range(self.num_latent):
            ...
            sum_val -= (normals.CholNormal(means[i, :], covar[i, :, :]).log_prob(means[i, :]) -
                    0.5 * trace)
            entropy = sum_val
        return entropy 
    ...
    def cross_entropy(self, means, covar, chol_var_weights, kernel_chol_weights):
        ...
        for i in range(self.num_latent):
            ...
            sum_val += (normals.CholNormal(means[i, :], kernel_chol_weights[i, :, :]).log_prob(0.0) -
                    0.5 * trace)
